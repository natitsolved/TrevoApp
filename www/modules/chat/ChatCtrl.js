app.controller('ChatCtrl', function ($scope, $stateParams, ionicMaterialInk, $ionicPopup, $timeout, authService, $state, $window, $cordovaSQLite, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $rootScope, $ionicLoading, $http, Upload, momentService) {
    ...
    $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ChatTable (id integer primary key, senderId integer, reciverId integer,message text,IsSender integer,imagePath text,videoPath text, IsCorrected integer, CorrectedText text,IncorrectText text,Notes text,IsFavourite integer,TranslatedText text)");
    ...
    $cordovaSQLite.execute(db, query, [$scope.userInfo.userId, $scope.recieverId, $scope.recieverId, $scope.userInfo.userId]).then(function (data) {
        ...
        if ($scope.hideShowRcvrTranslatedText == undefined) {
            $scope.hideShowRcvrTranslatedText = [];
        }
        for (var i = 0; i < data.rows.length; i++) {
            if (data.rows.item(i).IsCorrected == 1) {
                ...
                if (data.rows.item(i).TranslatedText) {
                    if (data.rows.item(i).IsSender == 1) {
                        $scope.hideShowTranslatedText[data.rows.item(i).id] = true;
                    }
                    else {
                        $scope.hideShowRcvrTranslatedText[data.rows.item(i).id] = true;
                    }
                }
    ...
    $scope.send_chat = function (data) {
        ...
        if ($scope.data != "") {
            ...
            data = { senderId: $scope.userInfo.userId, recieverId: reciverId, message: $scope.data, image: undefined, video: undefined, IncorrectText: undefined, Notes: undefined };
            ...
            $cordovaSQLite.execute($scope.db, query, [$scope.userInfo.userId, reciverId, $scope.data, 1, undefined, undefined, 0, undefined, undefined, undefined, 0, undefined]).then(function (res) {
    ...
    $rootScope.socket.on('chat_rcv', function (data) {
        ...
        if (data.recieverId == $scope.userInfo.userId) {
            ...
            var correctedText;
            var notes;
            if (data.IncorrectText) {
                incorrectText = data.IncorrectText;
                isCorrected = 1;
                correctedText = data.message;
                notes = data.Notes;
            }
            else {
            ...
            var query = "INSERT INTO ChatTable (senderId, reciverId,message,IsSender,imagePath,videoPath,IsCorrected,CorrectedText,IncorrectText,Notes,IsFavourite,TranslatedText) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
            $cordovaSQLite.execute($scope.db, query, [data.senderId, $scope.userInfo.userId, data.message, 0, data.image, data.video, isCorrected, correctedText, incorrectText, notes, 0, undefined]).then(function (res) {
                var message = "INSERT ID -> " + res.insertId;
                console.log(message);
                var url = data.video;
                if ($scope.recieverId == data.senderId) {
                    var pushData;
                    if (data.IncorrectText) {
                        //  var obj = { message: '', IsSender: 0, imgURI: data.image == null ? undefined : data.image, videoURI: data.video == null ? undefined : data.video, IncorrectText: incorrectText, CorrectedText: correctedText, inCorrectedPngSrc: "img/incorrect.png", correctedPngSrc: "img/ok.png", notes: notes, id: res.insertId };
                        $state.go('chat', {}, { reload: true });
                    }
                    else {
                        pushData = { message: data.message == null ? undefined : data.message, IsSender: 0, imgURI: data.image == null ? undefined : data.image, videoURI: data.video == null ? undefined : data.video, id: res.insertId };
                    }
                    $rootScope.ChatList.push(pushData);
        ...
        $cordovaCamera.getPicture(options).then(function (imageData) {
            ...
            $cordovaFile.copyFile(namePath, currentName, cordova.file.dataDirectory, newFileName).then(function (success) {
                ...
                $cordovaFileTransfer.upload(url, targetPath, options).then(function (result) {
                    ...
                    var reciverId = $scope.recieverId;
                    var query = "INSERT INTO ChatTable (senderId, reciverId,message,IsSender,imagePath,videoPath,IsCorrected,CorrectedText,IncorrectText,Notes,IsFavourite,TranslatedText) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
                    $cordovaSQLite.execute($scope.db, query, [$scope.userInfo.userId, reciverId, undefined, 1, url, undefined, 0, undefined, undefined, undefined, 0, undefined]).then(function (res) {
                        var message = "INSERT ID -> " + res.insertId;
                        console.log(message);
                        data = { senderId: $scope.userInfo.userId, recieverId: reciverId, message: undefined, image: url, video: undefined, IncorrectText: undefined, Notes: undefined, Notes: undefined };
                        data = JSON.stringify(data);
                        console.log(data);
                        $rootScope.socket.emit('chat_send', data);
                        $ionicLoading.hide();
            ...
            $cordovaFile.copyFile(namePath, currentName, cordova.file.dataDirectory, newFileName).then(function (success) {
                ...
                $cordovaFileTransfer.upload(url, targetPath, options).then(function (result) {
                    ...
                    $cordovaSQLite.execute($scope.db, query, [$scope.userInfo.userId, reciverId, undefined, 1, url, undefined, 0, undefined, undefined, undefined, 0, undefined]).then(function (res) {
                        var message = "INSERT ID -> " + res.insertId;
                        console.log(message);
                        data = { senderId: $scope.userInfo.userId, recieverId: reciverId, message: undefined, image: url, video: undefined, IncorrectText: undefined, Notes: undefined };
                        data = JSON.stringify(data);
                        console.log(data);
                        $rootScope.socket.emit('chat_send', data);
                        $ionicLoading.hide();
    ...
    $scope.uploadFile = function () {
        ...
        Upload.upload({
            ...
            $cordovaSQLite.execute($scope.db, query, [$scope.userInfo.userId, reciverId, undefined, 1, url, undefined, 0, undefined, undefined, undefined, 0, undefined]).then(function (res) {
                var message = "INSERT ID -> " + res.insertId;
                console.log(message);
                data = { senderId: $scope.userInfo.userId, recieverId: reciverId, message: undefined, image: undefined, video: url, IncorrectText: undefined, Notes: undefined };
                data = JSON.stringify(data);
                console.log(data);
                $rootScope.socket.emit('chat_send', data);
                $ionicLoading.hide();
        ...
        if ($window.localStorage["userInfo"]) {
            ...
            momentService.insertTransliterationDetails(item).then(function (data) {
                ...
                if (isSender == 1) {
                    ...
                    $scope.hideShowTranslatedText[$scope.transaltionObj.messageID] = true;
                }
                else {
                    ...
                    $scope.hideShowRcvrTranslatedText[$scope.transaltionObj.messageID] = true;
                }
    ...
    $scope.alerts = function (id, message, isSender, messageId, incorrectText, correctText) {
        $scope.transaltionObj = { id: id, message: message, isSender: isSender, messageID: messageId, IncorrectText: incorrectText, CorrectedText: correctText };
        $scope.show = false;
    }
    ...
    $scope.checkSpelling = function () {
        ...
        momentService.insertTransliterationDetails(item).then(function (data) {
            ...
            $rootScope.isSpellCheckFromChat = true;
            $ionicLoading.hide();
    ...
    $scope.markAsFav = function () {
        ...
        var isSender = $scope.transaltionObj.isSender;
        var item = { Message: message, FavouriteUserId: $scope.userInfo.userId, IsSender: 0, SenderRecieverId: $scope.recieverId, MomentId: 0, Details: message, LocalMessageId: $scope.transaltionObj.messageID };
    ...
    $scope.transliteration = function () {
        ...
        var transliterateObj = transl($scope.transaltionObj.message);
        var query = "Update ChatTable set TranslatedText= ? Where id=?";
        $cordovaSQLite.execute($scope.db, query, [transliterateObj, $scope.transaltionObj.messageID]).then(function (res) {
            if (isSender == 1)
                element = "translatedText_" + $scope.transaltionObj.id;
            else
                element = "rcvrTranslatedText_" + $scope.transaltionObj.id;
            var myElement = angular.element(document.querySelector('#' + element));
            myElement[0].innerHTML = transliterateObj;
            if (isSender == 1) {
                $scope.hideShowLoader[$scope.transaltionObj.id] = false;
            }
            else {
